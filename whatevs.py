...
def _run_tesseract(pil_img, whitelist):
    ...
    # Loop through PSM modes for robustness (restored from original script)
    # Added 'dan' language support
    for psm in (10, 8, 6, 7, 13):
        ...
        try:
            ...
            # Fallback to English/default if 'dan' is missing
...
if __name__ == "__main__":
    ...
    try: K, D = load_calib()
    except: print("No calibration file."); exit()

    processor = MonoRayCaster(K, D, SETUP_PITCH_DEG, CAMERA_HEIGHT_MM)
    img = cv2.imread("robot_pose_1.jpg")

    if img is not None:
        vis, data = processor.process(img)
        cv2.imshow("Monocular Result", vis)
